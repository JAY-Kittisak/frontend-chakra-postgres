...
import { useGiveByIdCdcQuery, useGiveByIdQuery, useMeQuery } from "../generated/graphql";
...
const GiveDetail: React.FC<Props> = () => {
    ...
    const [{ data: dataCdc, fetching: fetchingCdc }] = useGiveByIdCdcQuery({
        variables: {
            id: +gid,
        },
    });
    ...
    if (fetchingMe) {
        ...
        body = !dataCdc?.giveByIdCdc ? (
            ...
            >
                <Flex flexDir={["column", "column", "column", "row"]} align="center">
                    <Flex w={[null, null, null, "40%"]} justify="center" align="center">
                        {dataCdc.giveByIdCdc.imageUrl && (
                            <Image borderRadius="xl" boxSize="400px" src={dataCdc.giveByIdCdc.imageUrl} />
                            )}
                    </Flex>
                    <Flex flexDir="column" w={[null, null, null, "60%"]} justify="center">
                        <Flex p={5} flexDir="column" justifyContent="space-between">
                            <Center>
                                <Text fontSize="xl" fontWeight="bold">{dataCdc.giveByIdCdc.giveName}</Text>
                            </Center>

                            <Stack isInline mt={3} justify="space-between">
                                <Text fontSize={["sm", "sm", "md", "xl"]}>ราคา : </Text>
                                <Text fontSize={["sm", "sm", "md", "xl"]} fontWeight="semibold">{dataCdc.giveByIdCdc.price && formatAmount(dataCdc.giveByIdCdc.price)} บาท</Text>
                            </Stack>
                            <Divider orientation="horizontal" mt={3} />
                            ...
                            <Stack isInline mt={3} justify="space-between">
                                ...
                                <Text
                                    fontSize={["sm", "sm", "md", "xl"]}
                                    fontWeight="semibold"
                                    color={dataCdc.giveByIdCdc.inventory === 0 ? "red" : undefined}
                                >
                                    {dataCdc.giveByIdCdc.inventory} ชิ้น
                                </Text>Cdc
                            </Stack>
                            <Divider orientation="horizontal" mt={3} />
                            ...
                            <Stack isInline mt={3} justify="space-between">
                                <Text fontSize={["sm", "sm", "md", "xl"]}>ประเภท : </Text>
                                <Text fontSize={["sm", "sm", "md", "xl"]} fontWeight="semibold">{dataCdc.giveByIdCdc.category}</Text>
                            </Stack>
                            <Divider orientation="horizontal" mt={3} />
                            ...
                            <Flex ml={4}>
                                <Flex fontSize={["sm", "sm", "md", "xl"]} overflow="inherit" fontWeight="semibold">{dataCdc.giveByIdCdc.details}</Flex>
                            </Flex>
                            <Divider orientation="horizontal" mt={3} />

                            <Stack isInline mt={3} justify="space-between">
                                <Text fontSize={["sm", "sm", "md", "xl"]}>วันที่ซื้อ : </Text>
                                <Text fontSize={["sm", "sm", "md", "xl"]} fontWeight="semibold">{new Date(+dataCdc.giveByIdCdc.createdAt).toDateString()}</Text>
                            </Stack>
                            <Divider orientation="horizontal" mt={3} />
                ...
                <Center mt="5">
                    {dataCdc.giveByIdCdc.inventory === 0 ? (
                        <Center>
                            <Text mt={3} fontWeight="semibold" color="red" fontSize="2xl">
                                Out of stock
                            </Text>
                        ...
                        <>
                            ...
                            >
                                ...
                                <IconButton
                                    ...
                                    style={{
                                        cursor:
                                            amount === dataCdc.giveByIdCdc.inventory
                                                ? "not-allowed"
                                                : undefined,
                                    }}
                                    onClick={() =>
                                        setAmount((prev) => {
                                            if (prev === dataCdc.giveByIdCdc.inventory) return prev;
                                            return prev + 1;
                                        })
                                    }
                                />
                            ...
                            <Flex flexDir="column" alignItems="center">
                                <Button
                                    ...
                                    mr={3}
                                    bg={bgButton}
                                    disabled={dataCdc.giveByIdCdc.inventory === 0}
                                    // disabled={data.giveById.inventory === 250 || addToCartLoading}
                                    // loading={addToCartLoading}
                                    onClick={() => {
                                        setIsOpen(true);
